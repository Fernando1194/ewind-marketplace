          ...
          <div className="quote-box">
            ...
            {success && quoteData && (() => {
              ...
              const waMsg = encodeURIComponent(
                `Olá! Acabei de solicitar um orçamento pelo *Ewind* para o seu espaço *${space.name}*. 🎉\n\n` +
                `Seguem os detalhes do meu evento:\n\n` +
                `📌 *Tipo:* ${quoteData.eventType}\n` +
                `📅 *Data:* ${new Date(quoteData.eventDate + 'T12:00:00').toLocaleDateString('pt-BR', { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' })}\n` +
                `⏰ *Horário:* ${quoteData.eventTime ? quoteData.eventTime.substring(0,5) + 'h' : 'A combinar'}\n` +
                `👥 *Convidados:* ${quoteData.guestsCount} pessoas\n` +
                `⏱️ *Duração:* ${quoteData.duration} hora${parseInt(quoteData.duration) > 1 ? 's' : ''}\n` +
                (quoteData.message ? `\n💬 *Obs:* ${quoteData.message}\n` : '') +
                `\nMeu orçamento já foi enviado pela plataforma. Aguardo seu retorno pelo Ewind ou aqui mesmo. 😊`
              )
              return (
                <div style={{ background: '#f0fdf4', border: '1.5px solid #86efac', borderRadius: 12, padding: 18, textAlign: 'center', marginBottom: 12 }}>
                  ...
                  {waNumber && !whatsappSent ? (
                    <>
                      ...
                      <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 10, padding: '8px 12px', background: '#fff', border: '1px solid #e8e8e8', borderRadius: 8, lineHeight: 1.5 }}>
                        <div style={{ fontWeight: 700, color: '#2d2d2d', marginBottom: 4 }}>📱 Prévia da mensagem:</div>
                        <div style={{ fontStyle: 'italic', color: '#4b5563' }}>
                          "Olá! Acabei de solicitar um orçamento pelo <strong>Ewind</strong> para o seu espaço <strong>{space.name}</strong>. {quoteData.eventType} · {new Date(quoteData.eventDate + 'T12:00:00').toLocaleDateString('pt-BR')} {quoteData.eventTime ? `às ${quoteData.eventTime.substring(0,5)}h` : ''} · {quoteData.guestsCount} pessoas · {quoteData.duration}h"
                        </div>
                      </div>
                      <a
                        ...
                        style={{
                          ...
                          marginBottom: 8, boxShadow: '0 2px 8px rgba(37,211,102,0.3)'
                        }}
                      >
                        ...
                        Notificar anunciante pelo WhatsApp
                      </a>
                      <p style={{ fontSize: 10, color: '#9ca3af', textAlign: 'center', lineHeight: 1.5 }}>
                        Acelera a resposta em até 3x. O anunciante vê que o lead veio do Ewind.
                      </p>
                      <button onClick={() => setWhatsappSent(true)} style={{ fontSize: 12, color: '#9ca3af', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', display: 'block', margin: '4px auto 0' }}>
                        Pular — só aguardar pelo painel
                      </button>
                    </>
                  ) : (
                    <>
