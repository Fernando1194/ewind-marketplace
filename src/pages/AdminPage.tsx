                            ...
                            <tr key={s.id} style={{ borderBottom: '1px solid #f3f4f6', cursor: 'pointer', background: isExpanded ? '#fafffe' : '#fff' }} onClick={() => setExpandedSupplier(isExpanded ? null : s.id)}>
                              ...
                              <td style={{ padding: '10px 14px' }} onClick={e => e.stopPropagation()}>
                                <div style={{ display: 'flex', gap: 4 }}>
                                  ...
                                  {s.status === 'active' && <button onClick={() => updateSupplierStatus(s.id, 'paused', s.name)} disabled={actionLoading === s.id} style={{ padding: '3px 8px', fontSize: 10, fontWeight: 700, background: '#f3f4f6', border: 'none', borderRadius: 4, cursor: 'pointer', color: '#6b7280' }}>⏸</button>}
                                </div>
                              </td>
                            </tr>
                            {isExpanded && (
