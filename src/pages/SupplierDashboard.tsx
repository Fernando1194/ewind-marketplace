...
export default function SupplierDashboard({ user, goToPage }: Props) {
  ...
  // Feedback states
  const [activeSection, setActiveSection] = useState<'servicos' | 'orcamentos'>('servicos')
  ...
  const loadQuotes = async () => {
    ...
    if (supplierIds.length === 0) { setLoadingQ(false); return }
    const { data } = await supabase
      .from('quotes')
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '24px 28px' }}>
      ...
      <div style={{ display: 'flex', gap: 0, borderBottom: '2px solid #f0f0f0', marginBottom: 24 }}>
        {([
          { key: 'servicos',   label: '🛠️ Meus serviços' },
        ...
        <div>
          ...
          {!loadingQ && quotes.length === 0 && (
            <div style={{ textAlign: 'center', padding: '60px 24px', background: '#f9fafb', borderRadius: 14 }}>
              ...
              <p style={{ fontSize: 14, color: '#6b7280' }}>Quando alguém solicitar um orçamento para seus serviços, aparecerá aqui.</p>
            </div>
          )}
                ...
                <div style={{ padding: '14px 18px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', flexWrap: 'wrap', gap: 10 }}
                  ...
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 3 }}>
                      {q.suppliers?.name} <span style={{ color: '#6b7280', fontWeight: 400 }}>· {q.event_type}</span>
                    </div>
                    ...
                    {(q.status === 'pending' || q.status === 'viewed') && (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                        <div className="fg">
                          ...
                          <input type="number" value={replyPrice} onChange={e => setReplyPrice(e.target.value)} placeholder="Ex: 800" />
                        </div>
                        <div className="fg">
                          <label>Sua resposta *</label>
                          <textarea value={replyText} onChange={e => setReplyText(e.target.value)} rows={3}
                            placeholder="Olá! Tenho disponibilidade para a data solicitada..."
