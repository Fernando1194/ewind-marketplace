...
interface Props {
  ...
  reminderEventId?: string | null   // lembretes criados aqui pertencem a este evento (null = avulso)
  onChanged: () => void             // pede recarga ao pai após qualquer ação
...
export default function AgendaCalendar({ user, events, contracts, payments, reminders, reminderEventId = null, onChanged, openEvent, title = '📅 Agenda anual', subtitle = 'Clique em um dia para detalhes e ações.', headerActions }: Props) {
  ...
  const moveContract = async (ctId: string, newDate: string) => {
    ...
    setMovingCt(null)
    if (ok) { fb.showSuccess('Serviço remarcado.'); onChanged() }
  }
  const togglePaid = async (p: ContractPayment) => {
    ...
    if (ok) { fb.showSuccess(!p.paid ? 'Parcela marcada como paga.' : 'Parcela reaberta.'); onChanged() }
  }
  const addReminder = async () => {
    if (!rTitle.trim() || !openDay || savingReminder) return
    setSavingReminder(true)
    const ok = await fb.run(() => supabase.from('agenda_reminders').insert({ owner_id: user.id, event_id: reminderEventId, date: openDay, title: rTitle.trim(), notes: rNotes.trim() || null }))
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 14, marginBottom: 18 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: '#1a1a1a', marginBottom: 6 }}>{title}</h1>
            ...
            <div style={{ padding: '16px 22px', display: 'flex', flexDirection: 'column', gap: 10 }}>
              {dayEvents(openDay).map(ev => (
                <div key={ev.id} style={{ border: '1.5px solid #1a2e05', borderRadius: 12, padding: 14, background: '#f7fee7' }}>
                  ...
                  <button onClick={() => { setOpenDay(null); openEvent && openEvent(ev) }} className="btn-primary" style={{ padding: '8px 16px', fontSize: 13 }}>Abrir evento</button>
                </div>
              ))}
                ...
                return (
                  <div key={ct.id} style={{ border: '1px solid #e8e8e8', borderRadius: 12, padding: 14 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      {ev && openEvent && <button onClick={() => { setOpenDay(null); openEvent(ev) }} style={{ fontSize: 12, fontWeight: 600, padding: '7px 14px', borderRadius: 8, border: '1.5px solid #e8e8e8', background: '#fff', cursor: 'pointer', fontFamily: 'inherit', color: '#374151' }}>Abrir evento</button>}
